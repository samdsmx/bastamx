	...
	if ($Ano OR $IDCaso OR $Nombre OR $Telefono){
		...
		if (count($AutoridadReportada)>0 && $AutoridadReportada[0] <> "-"){
			...
			for ($i=0;$i<count($AutoridadReportada);$i++){
				$CadBusqueda .="c.AutoridadReportada = '".rs($AutoridadReportada[$i])."' ";
		...
		$sql ="create table $tmp SELECT DISTINCT l.*, c.Nombre, c.Telefono, c.Edad, c.Sexo, c.ComoTeEnteraste, c.AutoridadReportada, c.Municipio, c.Estado, c.EstadoCivil FROM Llamadas l, Casos c WHERE c.IDCaso=l.IDCaso $CadBusqueda2 $CadBusqueda Order By l.LlamadaNo Desc";
		$total_result = @mysql_query($sql, $connection) or die("Error #". mysql_errno() . ": " . mysql_error());
		}
	if(!$Correlacion){
		$Correlacion="IDCaso";
