...
if ($Sesion){
	...
	function Muestra($Tipo,$Clausula,$Order="Cantidad DESC",$Other=""){ 
		...
		if (count($AutoridadReportada)>0 && $AutoridadReportada[0] <> "-"){
			$CadBusqueda .="AND (";
			$criterio.="AutoridadReportada = ";			
			for ($i=0;$i<count($AutoridadReportada);$i++){
				$CadBusqueda .="c.AutoridadReportada = \"$AutoridadReportada[$i]\" ";
				$criterio.="$AutoridadReportada[$i] ";
				if ($i<count($AutoridadReportada)-1){
					$CadBusqueda .="OR ";
					$criterio.="o ";
					}
				}
	...
	$sql ="Create table Reporte select l.*, c.Edad,c.QuienContacta,c.Sexo,c.Municipio,c.EstadoCivil,c.Estado,c.AutoridadReportada,c.ComoTeEnteraste,c.MedioContacto from Casos c, Llamadas l where c.IDCaso=l.IDCaso $CadBusqueda2 $CadBusqueda";
	$total_result = @mysql_query($sql, $connection) or die("Error #". mysql_errno() . ": " . mysql_error());
	mysql_close($connection);
	...
	//QuienContacta
	$TotalQuienContacta=Muestra("QuienContacta","");
	...
	//AutoridadReportada
	$TotalOcu=Muestra("AutoridadReportada","");
